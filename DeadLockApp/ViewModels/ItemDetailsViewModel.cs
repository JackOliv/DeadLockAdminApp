...
namespace DeadLockApp.ViewModels
{
    public class ItemDetailsViewModel : BaseViewModel
    {
        ...
        {
            get => !string.IsNullOrEmpty(_fullImagePath) ? _fullImagePath : $"http://192.168.2.20/storage/{SelectedItem?.Image}";
            set
            {
        ...
        private async Task EditItemAsync()
        {
            ...
            try
            {
                ...
                var response = await _httpClient.PostAsync($"http://192.168.2.20/api/items/{SelectedItem.Id}", formData);
        ...
        private async Task DeleteItemAsync()
        {
            ...
            try
            {
                ...
                var response = await _httpClient.DeleteAsync($"http://192.168.2.20/api/items/{SelectedItem.Id}");
